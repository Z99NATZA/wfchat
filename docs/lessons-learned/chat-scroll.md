...
## 2026-07-29 - Near-bottom following trapped short touch timelines

Context:

- A short mobile conversation had less scrollable distance than the normal
  near-bottom threshold.

Failed approach:

- The same threshold kept following active and re-enabled it after user scrolling.
  Touch intent also existed only while the pointer id remained active.

Problem observed:

- Scrolling toward older messages snapped repeatedly back to the latest message,
  especially after native scrolling emitted `pointercancel`.

Root cause:

- Every position in the short timeline counted as near the bottom, while pointer
  cancellation discarded the evidence that the upward movement came from the
  user.

Lesson:

- Preserve touch intent briefly across pointer completion. Once upward user
  intent disables following, do not re-enable it during upward or bounce movement;
  require the viewport to return to the actual bottom.
